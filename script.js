...
class KernelPanicHandler {
    ...
    panic(code, message, errorObj, isFatal) {
        ...
        // ИСПРАВЛЕНО: Убрали errorObj?.stack
        const stack = (errorObj && errorObj.stack) ? errorObj.stack : "No Stack Trace";
...
class EnhancedSBTAModule {
    ...
    async getStatus() {
        ...
        let gps = null;
        try { gps = await this.getPreciseGPS(); } catch(e) {}
        ...
        return {
            ...
            gpsAvailable: !!gps,
            // ИСПРАВЛЕНО: Было gps?.accuracy, стало безопасная проверка
            gpsAccuracy: (gps && gps.accuracy) ? gps.accuracy : null,
            timeSynchronized: time.isSynchronized,
            timeDriftMs: time.timeDriftMs,
            calibrationPoints: this.calibrationData.size,
            quantumLattice: { size: this.quantumLattice.size, lastCrystallized: this.quantumLattice.generatedAt },
            recommendations: anchors.length === 0 ? ["Enroll an anchor first"] : ["Ready"]
        };
    }
}
...
class AppSandbox {
    ...
    async _handle(s, m, d) {
        ...
        if (s === 'hw') {
            check('hw');
            if (m === 'cap') {
                const img = this.kernel.UI.getROIImage();
                // ИСПРАВЛЕНО: Заменили img?.data на (img && img.data)
                return (img && img.data) ? Array.from(img.data) : null;
            }
        ...
        if (s === 'native') {
            if (m === 'hyper_start') {
                ...
                const loop = async () => {
                    ...
                    await this.kernel.HyperCore.step(1);
...
setTimeout(() => {
    ...
    if (buttonContainer && !document.getElementById('btn-solar-status')) {
        ...
        statusBtn.onclick = async () => {
            try {
                // Используем глобальный инстанс, чтобы точно не потерять контекст
                const k = window.KernelInstance;
                if(!k) return;

                k.UI.setStatus("SBTA: Checking status...", "#3498db");
                const status = await k.SBTA.getStatus();
                ...
                // БЕЗОПАСНАЯ ПРОВЕРКА (Вот тут была ошибка ?.)
                let gpsInfo = '✗';
                if (status.gpsAvailable) {
                    const acc = (status.gpsAccuracy !== null && status.gpsAccuracy !== undefined) ? status.gpsAccuracy.toFixed(1) : '?';
                    gpsInfo = `✓ (${acc}m)`;
                }
                ...
                statusWindow.querySelector('#debug-btn').onclick = () => {
                    ...
                    console.log('[SBTA] Calibration data:', k.SBTA.calibrationData);
                    alert('Debug info logged to console');
                };
                
                document.body.appendChild(statusWindow);
                k.UI.setStatus("SBTA: Status displayed", "#3498db");
...
window.addEventListener('error', e => {
    // ИСПРАВЛЕНО: Заменили Window.KernelInstance?.PanicHandler?.panic на безопасный if
    if (window.KernelInstance && window.KernelInstance.PanicHandler) {
        window.KernelInstance.PanicHandler.panic('UNCAUGHT_EXCEPTION', e.message, e, false);
    }
});
window.addEventListener('unhandledrejection', e => {
    // ИСПРАВЛЕНО: То же самое для промисов
    if (window.KernelInstance && window.KernelInstance.PanicHandler) {
        window.KernelInstance.PanicHandler.panic('PROMISE_REJECTION', e.reason ? e.reason.toString() : "Unknown", null, false);
    }
});
